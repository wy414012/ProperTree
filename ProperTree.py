...
class ProperTree:
    def __init__(self, plists = []):
        ...
        if str(sys.platform) == "darwin":
            ...
            file_menu.add_command(label="创建新窗口复制所有 (Cmd+D)", command=self.duplicate_plist)
            ...
            file_menu.add_command(label="OC 清理快照 (Cmd+Shift+R)", command=self.oc_clean_snapshot)
            file_menu.add_separator()
            file_menu.add_command(label="转换窗口 (Cmd+T)", command=lambda:self.show_window(self.tk))
            file_menu.add_command(label="清理注释 (Cmd+M)", command=self.strip_comments)
            file_menu.add_command(label="清理禁用条目 (Cmd+E)", command=self.strip_disabled)
            file_menu.add_separator()
            ...
            file_menu.add_command(label="切换 Plist/Data/Int 类型窗格 (Cmd+P)",command=self.hide_show_type)
            file_menu.add_separator()
